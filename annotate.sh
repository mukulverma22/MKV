...
export BRAKER_GFF3="$GFF_FILE"
export GENOME_FASTA="$FASTA_FILE"
...
run_annotation() {
    ...
    GFF_FILE=$(cd - > /dev/null && cd $(dirname "$GFF_FILE") && pwd)/$(basename "$GFF_FILE")
    FASTA_FILE=$(cd - > /dev/null && cd $(dirname "$FASTA_FILE") && pwd)/$(basename "$FASTA_FILE")
    
    # Source and run the main pipeline
    source "$CONFIG_FILE"
    ...
    # Run the actual pipeline (use the main script)
    bash "${SCRIPT_DIR}/functional_annotation_pipeline.sh" 2>&1
...
# Check if help/version only
if [ $# -eq 0 ] && [ -z "$GFF_FILE" ]; then
